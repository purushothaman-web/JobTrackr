...
const Profile = () => {
    ...
    const fetchData = async () => {
      ...
      try {
        const [profileRes, statsRes, activityRes, recentRes] = await Promise.all([
          api.get('/auth/me'),
          api.get('/jobs/stats'),
          api.get('/jobs/activity'),
          api.get('/jobs?page=1&limit=5&sortBy=updatedAt&order=desc')
        ]);
        setProfile(profileRes.data.data);
        setStats(statsRes.data.data);
        setActivityData(activityRes.data.data || {});
        setRecentJobs(recentRes.data.data.jobs || []);
      } catch (err) {
  ...
  const handleResendVerification = async () => {
    ...
    try {
      ...
      setMessage(res.data.data?.message || res.data.message || 'Verification email sent.');
    } catch (err) {
      setError(err.response?.data?.error || err.response?.data?.message || 'Failed to resend email.');
    } finally {
  ...
  const handleSendWeeklyReport = async () => {
    try {
      ...
      setMessage(res.data.message || res.data?.data?.message || 'Summary email sent.');
      setError('');
    } catch (err) {
      setError(err.response?.data?.error || err.response?.data?.message || 'Failed to send summary email.');
      setMessage('');
    }
  };
  ...
  const chartOptions = {
    ...
    plugins: { 
      ...
      tooltip: {
        ...
        bodyFont: { family: 'ui-monospace, SFMono-Regular, Menlo, Monaco, Consolas, "Liberation Mono", "Courier New", monospace', size: 12 },
        borderColor: '#27272a',
        borderWidth: 1,
        padding: 10,
        cornerRadius: 0,
    ...
    scales: {
      y: { 
        beginAtZero: true, 
        ticks: { stepSize: 1, font: { family: 'ui-monospace, SFMono-Regular, Menlo, Monaco, Consolas, "Liberation Mono", "Courier New", monospace', size: 10 }, color: '#71717a' },
        grid: { color: '#27272a' },
        border: { dash: [4, 4] }
      },
      x: {
        ticks: { font: { family: 'ui-monospace, SFMono-Regular, Menlo, Monaco, Consolas, "Liberation Mono", "Courier New", monospace', size: 10 }, color: '#71717a' },
        grid: { display: false }
      }
    },
  };
  ...
  if (!user) return (
     <div className="flex h-[60vh] items-center justify-center min-h-screen">
      <div className="font-mono text-zinc-500 uppercase tracking-widest">Unauthorized Access</div>
    </div>
  );

  return (
    <div className="w-full space-y-6 pt-8 pb-12 overflow-x-hidden">
      {/* Header Section */}
        ...
        <div className="bg-obsidian-light border border-border p-6 sm:p-8 flex flex-col">
          ...
          {loading ? (
            ...
            <div className="grid grid-cols-2 lg:grid-cols-3 gap-3">
              {statCards.map((item) => (
                <Link
                  to={item.label === 'Total Jobs' ? '/jobs' : `/jobs?status=${item.label.toLowerCase()}`}
                  key={item.label}
          ...
          <div className="overflow-x-auto w-full">
             <table className="w-full text-left font-mono text-[10px] sm:text-xs">
               <thead className="border-b border-border text-zinc-500 uppercase tracking-widest">
                 <tr>
                   <th className="px-4 py-3 font-normal whitespace-nowrap">Entity</th>
                   <th className="px-4 py-3 font-normal whitespace-nowrap">Designation</th>
                   <th className="px-4 py-3 font-normal text-right whitespace-nowrap">Status</th>
                   <th className="px-4 py-3 font-normal text-right whitespace-nowrap">Timestamp</th>
                 </tr>
               </thead>
               <tbody className="divide-y divide-border/50 uppercase">
                 {recentJobs.map((job) => (
                   <tr key={job.id} className="group hover:bg-obsidian transition-colors text-zinc-300">
                     <td className="px-4 py-4 truncate max-w-[120px] text-offwhite whitespace-nowrap">{job.company}</td>
                     <td className="px-4 py-4 truncate max-w-[150px] whitespace-nowrap">{job.position}</td>
                     ...
                     <td className="px-4 py-4 text-right text-zinc-500 whitespace-nowrap">
