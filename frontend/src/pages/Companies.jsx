...
const Companies = () => {
  ...
  const loadCompanies = useCallback(async () => {
    ...
    try {
      setLoading(true);
      const data = await fetchCompanies({ token: user.token, search: '' });
      setCompanies(data);
      setError('');
    } catch (err) {
  ...
  }, [authLoading, user, loadCompanies]);
  ...
  return (
    <div className="mx-auto grid w-full max-w-7xl gap-6 lg:grid-cols-[360px_1fr] py-8 px-4 sm:px-6">
      {/* Sidebar Form */}
      <section className="bg-obsidian-light border border-border p-6 h-fit lg:sticky lg:top-24">
        <div className="mb-8 border-b border-border pb-4">
          <h1 className="font-heading text-2xl font-black text-offwhite tracking-tighter uppercase">
            {editingId ? 'Edit Entity' : 'New Entity'}<span className="text-electric">.</span>
          </h1>
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="pt-4 flex flex-col gap-3">
            <Button type="submit" disabled={saving} className="w-full">
              {saving ? 'Processing...' : editingId ? 'Commit Changes' : 'Register Entity'}
            </Button>
            {editingId && (
              <Button
                variant="ghost"
