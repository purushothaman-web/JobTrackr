...
const JobDetails = () => {
  ...
  const loadAll = useCallback(async () => {
    ...
    try {
      ...
      const [jobData, interviewData, timelineData] = await Promise.all([
        fetchJob({ id, token: user.token }),
        fetchInterviews({ jobId: id, token: user.token }),
        getJobTimeline({ jobId: id, token: user.token }),
      ]);
      setJob(jobData);
  ...
  const addInterview = async (e) => {
    ...
    try {
      ...
      await createInterview({ jobId: id, payload: form, token: user.token });
  ...
  const setInterviewStatus = async (interviewId, status) => {
    if (!user?.token) return;
    try {
      await updateInterview({ interviewId, payload: { status }, token: user.token });
      await loadAll();
    } catch (err) {
  ...
  const removeInterview = async (interviewId) => {
    ...
    try {
      await deleteInterview({ interviewId, token: user.token });
      await loadAll();
    } catch (err) {
  ...
  const removeJob = async () => {
    ...
    try {
      await deleteJob({ id, token: user.token });
      navigate('/jobs');
    } catch (err) {
  ...
  if (!job && !loading) return (
    <div className="flex h-[60vh] items-center justify-center border border-dashed border-border p-12 mx-4 sm:mx-6">
  ...
  return (
    <div className="mx-auto w-full max-w-7xl space-y-6 px-4 sm:px-6 mb-12">
      ...
      <section className="bg-obsidian-light border border-border p-6 sm:p-8 relative overflow-hidden group">
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-6 relative z-10">
          ...
          <div className="flex sm:flex-col gap-3 w-full sm:w-auto">
            <Button variant="secondary" className="w-full sm:w-auto" onClick={() => navigate(`/edit-job/${job.id}`)}>
              Modify Matrix
            </Button>
      ...
      <div className="grid gap-6 lg:grid-cols-[1.2fr_1fr] items-start">
        ...
        {/* Timeline Column */}
        <section className="bg-obsidian border border-border p-6 h-fit lg:sticky lg:top-28">
